...
class LastFMCollector:
    ...
    def get_library_artist_info(self, username: str, limit: int = 1000):
        ...
        artist_names = [
            artist_dict[NAME_FIELD]
            for artist_dict in library_artist_results["artists"][ARTIST_FIELD]
        ]
        library_artist_info_results = self._get_artists_info_async(
    ...
    def _get_artist_info(
        ...
        if artist_mbid:
            request_url = self._construct_artist_query(
                MUSIC_BRAINZ_ID_FIELD, artist_mbid, response_format
            )
        elif artist_name:
            request_url = self._construct_artist_query(
                ARTIST_FIELD, artist_name, response_format
            )
    ...
    def _get_artists_info_async(  # NOTE: Make this take a list of artists rather than a single one.
        ...
        return send_async_requests(query_urls, MAX_CONCURRENT_REQUESTS)
